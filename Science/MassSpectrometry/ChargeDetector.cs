...
public static partial class Extensions
{
    ...
    private static void FindMonoMass1(List<point> data, out string result, out List<point> resultFit, out List<point> processedSignal)
    {
        System.Diagnostics.Debug.WriteLine("Executing FindMonoMass");

        double mass = data.First().X;

        var theoIon = MassSpec.GenerateAveragine(mass);
        ...
        var patternBins = pattern.RangeSum(pattern.MinKey, pattern.MaxKey, binSize);
        var signalBins = signal.RangeSum(signal.MinKey, signal.MaxKey, binSize);
